...
## Как это работает
### Юниты
При создании переменной `StoreUnit(new NewUnit())` копия `NewUnit` добавляется в список доступных для покупки юнитов. Там хранится по одному экземпляру каждого из юнитов. При покупке (выполнении действия `PurchaseUnit`) юнит "клонируется" методом `UnitCRTP<>::clone()` и новый экземпляр добавляет игрок-хозяин юнита.
### Действия
